...
| `TreeCursor` (**BUILT 2026-08-12**, provisional name — the carrier vote is Jason's) | **the carrier, reified**: the focused pair as one `readonly struct` (walkable + valid focus) — the type whose instances ARE Store-comonad values, arrived at from first principles (Jason: extract's outputs kept being pairs that existed as no type). THE INVARIANT: no unfocused cursor — "not yet positioned" is traversal-protocol state (`NodePosition.ForestRoot`, the treenumerator's before-first stance), not comonadic state; extract must always have a value, so `(tree, ∅)` is not a member of the carrier (Jason's one-node probe, confirmed via his ChatGPT dialogue). The CLR's `default` is documented invalid per the `ChildResult` convention. Co-Kleisli arrows become `Func<TreeCursor, TResult>`; `Duplicate()` is `Extend(cursor => cursor)` — extend of the identity, the definition, one line; steps return `TreeCursorResult` (a failed STEP yields no cursor, never a cursor standing nowhere); doors `CursorAt`/`GetRootCursor` demand a real focus. The `(tree, parentHandle)` "startHandle" alternative was refuted (parent-of-focus is not injective — a forest's `(tree, null)` identifies no node; the well-defined version needs a child index and IS `WalkableChildEnumerator`, the traversal citizen — the cursor/enumerator split, closed) | **carrier laws PINNED** (`TreeCursorLawTests`, first-run green): the counit as a TYPED IDENTITY — `cursor.Duplicate().Value` struct-equals the cursor, at every focus; duplicate commutes with steps (peek/seek coherence — "the labels are the refocusings," operationally); extract∘extend recovers the observer; the cursor sees UP (Store, not cofree, pinned on the carrier); the doors keep the no-unfocused invariant. Deep co-associativity inherited: cursor `Extend` delegates to the pinned walkable `Extend` |
