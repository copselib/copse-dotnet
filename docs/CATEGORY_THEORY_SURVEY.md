...
| `TakeSubtreesWhere` (main; landed post-fork, so this branch lacks it and the inventory missed it until 2026-08-12 — Jason's observation supplied the row, and his refinement corrected its first draft) | **the union of filtered duplicate's labels — a scan-derived filter, not a pour.** Two distinct operations live near this name. The FAMILY {subtree-at-m : p(m)} is `duplicate`'s labels filtered — one vantage per match, nested matches counted separately; on the walker that is literally `GetHandles().Where(h => p(GetValue(h)))`, vantages by reference. The OPERATOR returns the family's idempotent UNION: the source restricted to everything reachable from a match — nodes are excluded, never converted; one structure out. On a tree the downward closures form a *laminar family* (any two nested or disjoint), so the union is the forest of maximal members — the ratified OUTERMOST-MATCH-WINS rule (2026-08-06) is the union's tree shape, semantically emergent here exactly as the dag doc says it is for `TakeDownstreamWhere` ("closure union makes a nested match an interior node"); the tree side ratified as a rule what the semantics makes automatic. Derivation that classifies it: reachable-from-a-match is an INHERITED attribute — `RootfixScan(false, (flag, v) => flag ∨ p(v))`, then keep the flagged nodes, then matched roots promote. A comonadic observation consumed by a monadic reshaping — cross-tenant as a *pipeline*, in the scans' mold. The union-after-duplicate collapse is the join-shaped move, made overlap-safe by idempotence | dag transpose law pinned on `experimental/dag` (`TakeUpstreamWhere ≡ Transpose∘TakeDownstreamWhere∘Transpose`); the rootfix-flag derivation is **unpinned** — a natural reunification pin (`TakeSubtreesWhere(p) ≡ rootfix-OR → filter → promote`); the walker family is the FINER citizen (the union is derivable from it, not vice versa — the union forgets which matches were nested), so no equivalence is owed between them, only the derivation | zip / monoidal — **and the family REDUCES to one primitive** (discovered in the phase-2 close, 2026-08-12): `Intersection = Union.PruneBefore(!both)` (the *vanish* rule), `SymmetricDifference = Union.Where(!both)` and `Subtract = Union.Where(!HasRight).Select(.Left)` (the *promote* rule) — the Empty-graft fork of §6 is already living in the shipped set ops, one derived operator per rule | Union `Empty` identities + associativity up to reassociation + commutativity up to swap: **PINNED** (`CategoricalLawTests`); Intersection annihilation + commutativity up to swap: **PINNED**; Subtract right-identity + left-annihilator: **PINNED**; SymmetricDifference `Empty` identities both sides + commutativity up to swap: **PINNED**. **Non-law, documented:** Δ-associativity is NOT owed — tree-Δ rides the promote rule, promotion shifts positions, the set-theoretic xor law does not transfer to positional merges (same class as positional Where's non-composition) |
...
|---|---|---|
| `GetValue` | `extract` | **comonad laws PINNED** (2026-08-12, `WalkerComonadLawTests` — extract participates in all three: `extend(extract) ≡ id`, extract-after-extend recovers the observer, co-associativity; laws are joint equations, so this row was "pending" only until Extend existed to be the other party). The STORE-presentation `duplicate` (labels = bidirectional vantages) stays a derived one-liner — `w.Extend((w0, h) => h)`, the diagonal — awaiting a carrier type for a dignified surface name; the COFREE-presentation duplicate is SHIPPED, see the `Subtrees` row |
| `Subtrees()` (**BUILT 2026-08-12**, Jason's name — the surface speaks domain, the doc speaks theory, the `GetValue`/`Extend` bargain) | **the cofree duplicate**: every node relabeled with the subtree rooted at it — `Extend` of the severed re-rooted view (`SubtreeWalkable`: the region floor's first lens, landed as a label type — source handles shared, two answers rewritten, O(1) per label, lazy per pull). The presentation fork made shippable: cofree labels see only their FUTURE (leaffix-shaped observations), where Store vantages keep upward sight — which is why `Extend` remains the primitive and this is a derived view. Naming note: `Duplicate` was rejected for its two false .NET readings (English "copy"; MoreLINQ `Duplicates` = find-repeated-elements) | **cofree comonad laws PINNED nodewise** (`SubtreesLawTests`, first-run green): the subtree at a root ≡ that root's whole tree (extract∘duplicate); every label's root value ≡ the original value (map extract∘duplicate); a subtree of a subtree ≡ the deeper subtree (co-associativity); severance exactly at the root; outer shape ≡ source shape; interior labels pinned BY HAND against preorder ordinals (drawings, not code, wrote the expectations) |
| `TryGetParent` / `TryGetChildAt` / `TryGetRootAt` | the co-algebraic decoration (tree-structured position space); by-value probes, closed over handle-space with `GetValue` the only exit | closure/one-way-door: architectural (no value→handle member exists — the pledge by construction) |
...
| `Materialize` (absorbed `MaterializeWalkable`, 2026-08-13 — the buffer re-parent's collapse, OPEN-3 completed: EVERY capture is walkable, so the walker escalation is just declared-preorder Materialize) | **tabulate** (play the stream once, build the accessor — and the re-parent makes tabulation the universal comonad door for captures; growing memos tabulate INCREMENTALLY, probes as demand) | probe idempotence pinned; adjacency ≡ oracle pinned for every buffer producer (`BufferAdjacencyConformanceTests`, 7 providers × both handle spaces incl. the mid-race memo); the comonad law suites run over the provider fan-out, so tabulate's output is law-certified per citizen |
...
| `TreeWalker` (**BUILT 2026-08-12**; **NAME RATIFIED 2026-08-12** — Jason's vote, closing the carrier-name question: the 2016 library's `ITreeWalker` named the *topology* "walker" (a misnomer by his own later reclassification), and the name now migrates home to the agent — `IWalkableTreenumerable` is walk-*able* (topology, passive), the `TreeWalker` walks it (stance, active); DOM precedent (`createTreeWalker` = the positional navigator) agrees; `DagWalker` is the planned dag-branch sibling, with edge-atomic steps) | **the carrier, reified**: the focused pair as one `readonly struct` (walkable + valid focus) — the type whose instances ARE Store-comonad values, arrived at from first principles (Jason: extract's outputs kept being pairs that existed as no type). THE INVARIANT: no unfocused cursor — "not yet positioned" is traversal-protocol state (`NodePosition.ForestRoot`, the treenumerator's before-first stance), not comonadic state; extract must always have a value, so `(tree, ∅)` is not a member of the carrier (Jason's one-node probe, confirmed via his ChatGPT dialogue). The CLR's `default` is documented invalid per the `ChildResult` convention. Co-Kleisli arrows become `Func<TreeWalker, TResult>`; `Duplicate()` is `Extend(cursor => cursor)` — extend of the identity, the definition, one line; steps return `TreeWalkerResult` (a failed STEP yields no cursor, never a cursor standing nowhere); doors `GetTreeWalkerAt`/`TryGetTreeWalkerAtRootIndex` (Try-law names, 2026-08-14) demand a real focus. The `(tree, parentHandle)` "startHandle" alternative was refuted (parent-of-focus is not injective — a forest's `(tree, null)` identifies no node; the well-defined version needs a child index and IS `WalkableChildEnumerator`, the traversal citizen — the cursor/enumerator split, closed) | **carrier laws PINNED** (`TreeWalkerLawTests`, first-run green): the counit as a TYPED IDENTITY — `cursor.Duplicate().Value` struct-equals the cursor, at every focus; duplicate commutes with steps (peek/seek coherence — "the labels are the refocusings," operationally); extract∘extend recovers the observer; the cursor sees UP (Store, not cofree, pinned on the carrier); the doors keep the no-unfocused invariant. Deep co-associativity inherited: cursor `Extend` delegates to the pinned walkable `Extend` |
...
adjacency-bound.** Store's peek is total — a vantage is (topology, focus) where the topology
answers for *every* position — and the shipped walkable `Extend` observer receives
`(source, handle)`, the whole topology, which is why the leaffix coherence pin could exist
...
**One comonad, over focused topology. Many topology *representations*. Extend has many
...
- **The carrier**: `TreeWalker` is the focused pair — the honest type of "a place in a
  ...
  never do); the algebra's home is the topology. The Store-vs-cofree distinction (does a
...
- **Admission**: the skeleton is a lawful carrier representation, not an implementation
  ...
  topology (the raw store rewrapped with nothing else, validity-checked on the way in), so
