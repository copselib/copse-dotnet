...
| TakeSubtreesWhere | D (streams); **B**(documented capture); F | D: same-dim narrow; B/F: ITreenumerableBuffer | D streams / B+F capture(deferred-once) | **THE SUBGRAPH SELECTION CLUSTER's tree restriction (2026-08-06)**: matched subtrees re-rooted as the result forest — match depth compresses to 0, descendants keep sibling indices (depth shifted), roots take source preorder match order (sibling 0, 1, ...); **OUTERMOST MATCH WINS, as a rule** (the in-subtree flag: inside a match the predicate never fires — trees cannot share substructure, so nested matches suppress; the dag analog `TakeSubgraphsWhere` on experimental/dag gets the same semantics EMERGENTLY from induced in-degree, and is the general form); predicate arity-split value \| positional (source labels, the positional-Where rule); D-narrow arm is an O(1)-state pass-through wrapper (a matched subtree is one contiguous preorder segment); the result's BFT cannot stream (matches start at different source depths — the reorder wall), so F builds a RESULT-sized preorder store from one depth-first walk (buffer disclosure) and B Materializes first (source-sized capture, then the same build) |
...
| Do / Hide | F, D, B | same-dim | streams | O(1); Do = the sanctioned effect point: action per emitted visit, receives the full NodeVisit (deliberately permissive — narrower cadences are caller-side filters); keeps NodeVisit through the signature migration; NEVER composes and prevents composition across it by definition (composition design: the window materializes the pane); **THE LANDING IDIOM (the Do-quartet demotion, 2026-08-04 — docs/SCANRESULT_DESIGN.md THE DEMOTION)**: mutable-node aggregation = pure op . Do(scheduling-mode-filtered landing) . Select(.Node) — effects per drain (the re-enumeration contract), Materialize/Memoize is the consumer's pin; the quartet (Rootfix|Leaffix)Do(Scan|Dispatch) is DELETED (derivable sugar + operator-pinned effects); admission bar for any Do variant: a workload the mode filter cannot serve; pinned by DoLandingCompositionTests |
| RootfixScan (seed / rootNodeSelector) | F, D, B | same-dim | streams | O(depth) DFT / O(width) BFT; seed/selector precedes the accumulator (type-fixer-first, 2026-08-02); **ScanResult sweep 2026-08-02**: returns the canonical pairing (`ITreenumerable<NodeAccumulation<TNode,TAcc>>` since the 2026-08-06 type-level recording rule; born `ScanResult` — project `.Accumulate` for values), `NodeContext<TAccumulate>` retired, selector arity-split value \| positional, engine treenumerators untouched (instantiated with TAccumulate = the pairing); **SEAT RULE 2026-08-04** (the alpha.9 verdict): accumulator is `(TAcc, TNode)` — Aggregate's shape, the minimal basis, the shape every effect callback shares via the landing idiom (ancestry rides the state — a parameter earns a seat iff the caller cannot derive it; a parent-centric rule is a survey)); **THE NORTH STAR 2026-08-05** (cross-tier flavor coherence; reversed the one-day arrival-semantics detour): seed = the virtual root's arrival, folded at EVERY node (accumulator(seed, root)); rootNodeSelector = each root's ACCUMULATION set directly, fold bypassed at roots (the bypass instrument, mirroring the dispatch selector) — seed ≠ constant selector, pinned deliberately-different on BOTH tiers; Scan(boundary, fold) ≡ fold-encoded Dispatch(boundary) for every flavor (CrossTierCoherenceTests) |
...
| Materialize | F(±strategy), D, B | ITreenumerableBuffer | **capture(deferred-once)** | **LAZY 2026-08-10** (was eager; async surface renamed `MaterializeAsync`→`Materialize` — no longer awaitable, suffix dropped): construction pinned to the first pull through the lazy store's grow seam (the LeaffixScan/Invert cost shape); the law — *construction uniformly lazy, the pin a commitment at the earliest free moment*. Organic: THE FIRST CONSUMER PINS THE LAYOUT (DFT-first → preorder, BFT-first → level-order; dimension-dispatched `Tree.Lazy`). Strategy overload: the layout GUARANTEE stands (2026-07-15, never ignored) and the pin lands AT THE CALL — a live memo's capture is created for the dimension with zero nodes pulled, so an intervening consumer cannot re-pin a shared memo; the O(n) waits for the first pull. Probes unchanged in spirit: a live memo gets the completion seam (`MaterializeTreenumerable` — completes IN BULK at the first pull, feed retires there; the result is non-disposable, the memo's disposal stays the caller's); a compliant buffer returns as-is; a mismatched/undecided one is TRANSPOSED from the buffer at the first pull. An unconsumed result holds exactly what the unconsumed pipeline held. The both-layouts recipe survives = materialize twice, one source pass. Benchmark rows force the build (Materialize rows one-pull in-method; Memoize/Serialization settle in setup) |
