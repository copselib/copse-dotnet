...
| RootfixScan (seed / rootNodeSelector) | F, D, B | same-dim | streams | O(depth) DFT / O(width) BFT; seed/selector precedes the accumulator (type-fixer-first unification 2026-08-02: args that fix a type parameter precede the lambdas that consume it — BCL Aggregate's order; RootfixDispatch already had it, the scan/aggregate tier joined) |
| RootfixDoScan (SPIKE, feature/do-scan) | F, D, B | same-dim, PASS-THROUGH (`ITreenumerable<TNode>` — no projection; `TAccumulate` is internal plumbing) | streams; **IMPURE BY DECLARATION** | The Do idiom, scan-shaped (2026-08-02): `compute` (pure, scan's permissive unspecified-counts clause) threads the seed down; `store` — the declared effect point — fires EXACTLY once per node per traversal, receiving the (node, accumulation) pairing; effects refire per drain BY CONTRACT (each dimension's drain is a traversal) — `Memoize`/`Materialize` are the pinning composition; input/output field separation is the natural idiom (re-runnable passes for free); composition barrier like Do; selector overloads joined 2026-08-02 (value \| positional, the family grammar; on this STREAMING Do tier the selector is the freshness form — fires per root per traversal, closures read live state; roots take the selector directly, compute never sees a fabricated arrival); implementation deliberately stays on the scan machinery, NOT the shared dispatch pass (ruled 2026-08-02: fold tier streams, survey tier captures — delegating would demote to capture-class); spike rides the pure scan's machinery (both treenumerators invoke the accumulator once per node at scheduling) |
...
| LeaffixDoDispatch (SPIKE, feature/do-scan) | D; **B**; F(→D) | same-dim, PASS-THROUGH (`ITreenumerableBuffer<TNode>`) | capture(deferred-once); **IMPURE BY DECLARATION** | LeaffixDispatch's Do twin (2026-08-02), the mutable-workload rollup: `(seed \| leafNodeSelector flavors, survey, store)` — survey PURE and value-flavored over the `ChildAccumulations` view, `store` the declared effect point (once per node per build, preorder, total coverage: leaves via seed/selector, internal via survey); rides the shared fold pass with a pass-through values sink (reuses the pass's subtree-size array; adds only the values array the pure operator never needed); composition barrier like Do |
...
| RootfixAggregate (seed / selector) | D, B, F(→D) | IEnumerable | streams | RootfixScan + GetLeaves; seed/selector first (type-fixer-first, 2026-08-02) |
