...
| Materialize | F(±strategy), D, B | ITreenumerableBuffer | **capture(eager)** | probes first (2026-07-13): a live memo is consumed in place; a compliant buffer returned as-is; otherwise `Memoize()+Consume()`. The strategy overload is a layout GUARANTEE (2026-07-15): never ignored — fresh memo pins it, mismatched buffer is TRANSPOSED from the buffer (new instance, source untouched); the both-layouts recipe = materialize twice, one source pass. Consume(strategy) stays advisory (documented as a suggestion) |
...
| Consume | F(±strategy), D, B | void | drains / settles | probes (2026-07-14, mirroring Materialize): lazy buffer → Complete() its one capture (the member renamed 2026-07-15: the extension DRAINS, the member FINISHES BUILDING — IsComplete/Complete symmetry; parameterless, since completing the capture is all it can mean; the extension's suggestedStrategy pins a fresh buffer via acquisition, and layout guarantees are Materialize's job); completed buffer → no-op (inertness is an `ITreenumerableBuffer` contract obligation; deferred captures stay deferred); plain tree → drain (benchmarks and forced walks use explicit `Drain` helpers) |
