...
| LeaffixAggregate (seed \| leafNodeSelector) | D; **B**(documented capture); F(→D) | lazy IAsyncEnumerable\<ScanResult\> (one per root) | per-root lazy (DFT); capture (BFT) | **value-flavored on the dual shape 2026-08-05** — `(seed \| leafSelector \| positional, edgeAccumulator, nodeAccumulator)`, the family's last NodeContext callbacks retired (the deferred signature workstream closed); mechanism = LeaffixScan's node-last fold collapsed to roots; per-root laziness (DFT) and the index-chasing BFT capture preserved |
...
|---|---|---|---|---|
| PruneAfter lens | IWalkableTreenumerable | pair-citizen view | lazy | stream half delegates to the streaming operator; adjacency half wraps the probe (the lens family; crossed colors 2026-08-14 — async source, generated sync twin) |
