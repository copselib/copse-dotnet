...
| RootfixScan (seed / rootNodeSelector) | F, D, B | same-dim | streams | O(depth) DFT / O(width) BFT; seed/selector precedes the accumulator (type-fixer-first, 2026-08-02); **ScanResult sweep 2026-08-02**: returns the canonical pairing (`ITreenumerable<ScanResult<TNode,TAcc>>` — project `.Accumulate` for values), `NodeContext<TAccumulate>` retired, selector arity-split value \| positional, engine treenumerators untouched (instantiated with TAccumulate = the pairing); **SEAT RULE 2026-08-04** (the alpha.9 verdict): accumulator is `(TAcc, TNode)` — Aggregate's shape, the minimal basis, the shape every effect callback shares via the landing idiom (ancestry rides the state — a parameter earns a seat iff the caller cannot derive it; a parent-centric rule is a survey)); **ARRIVAL SEMANTICS 2026-08-04** (full participation on the fold tier): the rootNodeSelector supplies the root's ARRIVAL and the fold fires at EVERY node — root accumulate = accumulator(selector(root), root), the seed flavor is the constant selector, invocation sentinel = n (was n − roots) |
