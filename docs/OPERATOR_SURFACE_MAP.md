...
| LeaffixScan (seed \| leafNodeSelector) | D; **B**; F(→D) | ITreenumerableBuffer\<ScanResult\> | capture(deferred-once) | **THE DUAL RESHAPE 2026-08-05** (docs/SCANRESULT_DESIGN.md THE LEAFFIX DUAL): `(seed \| leafSelector \| positional, Func<TAcc,TAcc,TAcc> edgeAccumulator, Func<TAcc,TSource,TAcc> nodeAccumulator)` — value(n) = nodeAcc(edgeReduce(children), n); the nodeAccumulator IS RootfixScan's fold shape; the seed = the VIRTUAL FRINGE's arrival, participating (value(leaf) = nodeAcc(seed, leaf)); selector = bypass; seed ≠ constant selector, pinned; the old map-then-combine (map fused boundary+contribution) is DELETED, ternary context flavor awaits an edgeAccumulator workload; sugar over LeaffixDispatch's internal pass (CrossTierCoherenceTests pins both flavor pairs) |
