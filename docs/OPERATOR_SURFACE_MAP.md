...
| LeaffixDoDispatch (SPIKE, feature/do-scan) | D; **B**; F(→D) | same-dim, PASS-THROUGH (`ITreenumerableBuffer<TNode>`) | capture(deferred-once); **IMPURE BY DECLARATION** | LeaffixDispatch's Do twin (2026-08-02), the mutable-workload rollup: `(seed \| leafNodeSelector flavors, survey, store)` — survey PURE and value-flavored over the `DispatchSources` view, `store` the declared effect point (once per node per build, preorder, total coverage: leaves via seed/selector, internal via survey); **delivery model 2026-08-04**: all landings happen together when the fold completes — all-or-nothing effects; rides the shared fold pass with a pass-through values sink (reuses the pass's subtree-size array; adds only the values array the pure operator never needed); composition barrier like Do |
