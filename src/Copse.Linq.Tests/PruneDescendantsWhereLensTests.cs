...
{
  // The restriction lens's pins: PruneDescendantsWhere over a walkable stays walkable (statically -- the
  ...
  public class PruneDescendantsWhereLensTests
  {
    ...
    [TestMethod]
    public void PruneDescendantsWhereLens_OrderHalf_MatchesTheStreamingOracle_BothDimensions()
    {
      var lensed = TreeSerializer.DeserializeDepthFirstTree(ToyTree)
        .Materialize(BufferLayout.Preorder)
        .PruneDescendantsWhere(value => value == "b");

      var oracle = TreeSerializer.DeserializeDepthFirstTree(ToyTree).PruneDescendantsWhere(value => value == "b");
    ...
    [TestMethod]
    public void PruneDescendantsWhereLens_StacksWithoutALattice()
    {
      ...
      var stacked = TreeSerializer.DeserializeDepthFirstTree(ToyTree)
        .Materialize(BufferLayout.Preorder)
        .PruneDescendantsWhere(value => value == "b")
        .PruneDescendantsWhere(value => value == "c");
