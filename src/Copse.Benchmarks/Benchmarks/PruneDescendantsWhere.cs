...
namespace Copse.Benchmarks
{
  ...
  public class PruneDescendantsWhere
  {
    private const int HalfDepth = 724;

    [Benchmark]
    public void Dft_Forest_All() =>
      CanonicalTrees.MegaForest().PruneDescendantsWhere(_ => true).Consume(TreeTraversalStrategy.DepthFirst);

    [Benchmark]
    public void Bft_Forest_All() =>
      CanonicalTrees.MegaForest().PruneDescendantsWhere(_ => true).Consume(TreeTraversalStrategy.BreadthFirst);

    [Benchmark]
    public void Dft_Triangle_HalfDepth() =>
      CanonicalTrees.MegaTriangleTree()
      .PruneDescendantsWhere((n, position) => position.Depth == HalfDepth)
