...
{
  // The erased combinator surface of a composed chain's accumulated mapping (the CompositionMap
  ...
  // treenumerator; a filtering map acquires through the result driver). Erasure is why this
  ...
  internal interface ICompositionMap<TNode>
  {
    ...
    // The filter-bind: compose a type-preserving result stage (where, prunes, future
    // filters). relabels: whether THIS stage moves surviving nodes' labels (Where and
    // PruneBefore do; PruneAfter does not).
    ICompositionMap<TNode> Filter(Func<NodeContext<TNode>, CompositionResult<TNode>> stage, bool relabels);
