...
namespace Copse.Linq.Stores
{
  ...
  // replay's frontier. The structural dual of MemoizePreorderCapture, LOUDS-adjacent where that
  ...
  //
  // The front's span updates inline through the ref-returning _ChildCounts indexer rather than a
  // `ref var frontChildCount` local -- by-reference locals are illegal in async methods (CS8177),
  // and the sync twin transcribes the same shape (the indexer ref-returns the same slot each call,
  ...
  internal sealed class AsyncMemoizeLevelOrderCapture<TNode> : IAsyncDisposable
  {
    public AsyncMemoizeLevelOrderCapture(Func<IAsyncTreenumerator<TNode>> feedFactory)
    {
    ...
    {
      public Handle(AsyncMemoizeLevelOrderCapture<TNode> buffer)
      {
        _Buffer = buffer;
      }
