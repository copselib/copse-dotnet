...
namespace Copse.Linq
{
  public static partial class AsyncTreenumerable
  {
    ...
    /// <see cref="ChildAccumulations{TAccumulate}"/> view; leaves take <paramref name="seed"/>
    ...
    ///
    /// <para>Rides the pure operator's shared fold pass with a pass-through values sink; the
    /// result reuses the pass's subtree-size array, so the Do twin adds only the values array
    /// the pure operator never needed.</para>
    /// </summary>
    public static IAsyncTreenumerableBuffer<TSource> LeaffixDoDispatch<TSource, TAccumulate>(
      ...
      TAccumulate seed,
      Func<TSource, ChildAccumulations<TAccumulate>, TAccumulate> survey,
    ...
    // The Do finisher over the shared fold pass: where the pure build's result IS the
    // accumulation tree, this one hands each (value, accumulation) pair to store -- preorder
    // order after the fold completes, exactly once per node -- and returns the pass-through
    // buffer over the values sink, reusing the pass's subtree-size array.
    private static async ValueTask<AsyncPreorderArrayStore<TSource>> BuildLeaffixDoDispatchAsync<TSource, TAccumulate>(
      ...
      Action<TSource, TAccumulate> store)
    {
      var passThroughValues = new List<TSource>();
      var (accumulations, subtreeSizes) = await RunLeaffixDispatchPassAsync(
          source,
          leafContext => leafNodeSelector(leafContext.Node, leafContext.Position),
          (nodeContext, children) => survey(nodeContext.Node, children),
          passThroughValues)
        .ConfigureAwait(false);

      var values = passThroughValues.ToArray();
