...
// syntax adapted to C# 7.3 (nullable annotations, switch expressions, target-typed new
// removed) and Rx's resource string inlined. Semantics unchanged, including Rx's
// thread-safety (retained deliberately even though Copse is single-threaded by contract)
// and the load-bearing behaviors: Add on a disposed composite disposes the item
// immediately; children dispose in add order.
...
namespace Copse.Disposables
{
  ...
  public sealed class CompositeDisposable : ICollection<IDisposable>, ICancelable
  {
    ...
    private const string DisposablesCannotContainNull = "Disposables collection can not contain null values.";
    ...
    public CompositeDisposable(params IDisposable[] disposables)
    {
      ...
      var listOrDictionary = ToListOrDictionary(disposables);

      _disposables = listOrDictionary.Collection;
    ...
    public CompositeDisposable(IEnumerable<IDisposable> disposables)
    {
      ...
      // _count can be read by other threads and thus should be properly visible
      // also releases the _disposables contents so it becomes thread-safe
      Volatile.Write(ref _count, listOrDictionary.Count);
    }
    ...
    private static (object Collection, int Count) ToListOrDictionary(IEnumerable<IDisposable> disposables)
    {
      ...
      if (disposables is IDisposable[] array)
      {
        capacity = array.Length;
      }
      else if (disposables is ICollection<IDisposable> collection)
      {
      ...
      if (capacity > MaximumLinearSearchThreshold)
      {
        ...
        foreach (var d in disposables)
        {
          if (d == null)
          {
            throw new ArgumentException(DisposablesCannotContainNull, nameof(disposables));
          }
    ...
    public void Dispose()
    {
      ...
      if (currentDisposablesList != null)
      {
        foreach (var d in currentDisposablesList)
        {
          // Although we don't allow nulls in from the outside, we implement Remove
      ...
      if (currentDisposablesDictionary != null)
      {
    ...
    public bool Contains(IDisposable item)
    {
      ...
      lock (_gate)
      {
        ...
        if (current is List<IDisposable> list)
        {
          return list.Contains(item);
        }

        return ((Dictionary<IDisposable, int>)current).ContainsKey(item);
      }
    }
    ...
    public IEnumerator<IDisposable> GetEnumerator()
    {
      lock (_gate)
      {
        ...
        if (current is List<IDisposable> currentList)
        {
          return new CompositeEnumerator(currentList.ToArray());
        }

        return new CompositeEnumerator(((Dictionary<IDisposable, int>)current).Keys.ToArray());
      }
    }
