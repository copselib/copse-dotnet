...
{
  /// <summary>
  /// Depth-first treenumerator over a preorder store: native playback for the flat family
  /// (trees stored as linear preorder encodings) the way
  /// <see cref="DepthFirstTreenumerator{TValue, TNode, TChildEnumerator}"/> is the traversal for
  /// the hierarchical family (trees adapted via the child-enumerator protocol). Emits the exact
  /// engine visit-stream contract -- same modes, visit counts, raw positions, and
  /// <see cref="NodeTraversalStrategies"/> semantics -- but synthesizes it from span arithmetic
  /// instead of child enumerators: a node's first child is the next store index, a child's next
  /// sibling is the child's index plus its (ensured-closed) subtree size, and a candidate belongs
  /// to its parent while the parent's span is open or contains it.
  ///
  /// <para>Natural traversal reads the store strictly sequentially (each next node is the next
  /// index; closes were ensured just in time), so playback recreates no structure and touches no
  /// delegates. A consumer skip is a span hop, paying exactly the store's cost to close the
  /// hopped subtree (for a growing store, the eager-skip fill; for a completed one, an array
  /// read).</para>
  ///
  /// <para>The driver logic deliberately mirrors the engine's
  /// (OnScheduling/OnVisiting/Backtrack, the depth-of-last-visited-node classify), because the
  /// contract subtleties -- SkipNode child promotion, SkipSiblings' skipped-ancestor walk,
  /// between-children visits -- live in that structure. The path here is one deque of levels
  /// (skipped levels stay resident, flagged, to promote their children) rather than the engine's
  /// two, since a flat level has no child enumerator to keep small.</para>
  /// </summary>
    ...
    where TStore : IPreorderStore<TValue>
  {
    ...
    private bool TryPushNextChild()
    {
      ...
      top.LastChildIndex = candidate;

      var position = new NodePosition(top.NextSiblingIndex++, top.Position.Depth + 1);
