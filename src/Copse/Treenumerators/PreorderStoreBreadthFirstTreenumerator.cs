...
{
  /// <summary>
  /// Breadth-first treenumerator over a preorder store: the CROSS-ORDER member of the flat
  /// family's BFT pair (its native twin is
  /// <see cref="LevelOrderStoreBreadthFirstTreenumerator{TValue, TStore}"/>). Same driver,
  /// different child arithmetic: a node's first child is the next store index and each next
  /// sibling is the previous child's index plus its (ensured-closed) subtree size, so the visit
  /// queue holds scattered store indices and traversal pays the cross-order locality tax --
  /// correct and O(width), but without the native pair's sequential read. This is the traversal
  /// a completed preorder capture uses to serve the other dimension (the memo's four-case rule,
  /// case 2).
  ///
  /// <para>Emits the exact engine visit-stream contract; see the native twin for the driver
  /// structure (schedule stack + visit queue + slot carry).</para>
  /// </summary>
    ...
    where TStore : IPreorderStore<TValue>
  {
    ...
    // Produce the next single visit (or false when the traversal is exhausted). See the native
    ...
    // child) or the previous child's index plus its ensured-closed subtree size (next sibling);
    // it belongs to this parent while the parent's span is still open or contains it.
    ...
    private bool TryScheduleNextChildOf(ref Frame parent)
    {
      ...
      parent.LastChildIndex = candidate;

      var position = new NodePosition(parent.NextSiblingIndex++, parent.Position.Depth + 1);
