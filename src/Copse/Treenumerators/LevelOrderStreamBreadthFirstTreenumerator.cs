...
{
  /// <summary>
  /// Breadth-first treenumerator over a forward-only level-order stream: the streaming tier of
  /// <see cref="LevelOrderStoreBreadthFirstTreenumerator{TValue, TStore}"/>. Same driver
  /// (schedule stack + visit queue + slot carry), but the source affords one pass, so parsed
  /// content lives in a sliding WINDOW of entries that grows at the parse cursor and evicts as
  /// visit-queue fronts retire -- O(width) resident state, the frontier the visit contract
  /// already implies.
  ///
  /// <para>The encoding's positions are load-bearing (group k+1 belongs to the k-th streamed
  /// node), which shapes the two subtleties here:</para>
  /// <list type="bullet">
  /// <item><b>Pruned nodes still own groups.</b> A consumer skip cannot erase a node from the
  /// stream, so its entry is flagged and, when its group's turn comes, the group is discarded
  /// UNMAPPED and its entry count appended as further flagged entries -- the cascade that keeps
  /// group/owner alignment intact without materializing a single pruned value.</item>
  /// <item><b>SkipNode promotion reads ahead.</b> A promoted child's children are owed
  /// immediately, but its group sits behind the intervening nodes' groups; the parser advances
  /// through them, buffering their (soon needed anyway) children in the window -- bounded by the
  /// frontier, never the tree.</item>
  /// </list>
  ///
  /// <para>The treenumerator OWNS the stream and disposes it -- the reader-ownership hook the
  /// serializer's contract needs.</para>
  /// </summary>
    ...
    where TStream : ILevelOrderStream<TValue>
  {
    ...
    // THE SEAM, windowed edition: the given parent's next child is child ordinal
    // NextChildOrdinal, once the parser has advanced far enough to prove it exists.
    ...
    private bool TryScheduleNextChildOf(ref Frame parent)
    {
      ...
      if (!TryEnsureChild(parent.NodeIndex, ordinal))
        return false;

      var childIndex = GetEntry(parent.NodeIndex).FirstChildIndex + ordinal;
    ...
    private bool TryEnsureRoot(int k)
    {
      while (_RootCount <= k)
      {
    ...
    private bool TryEnsureChild(int parent, int k)
    {
    ...
    private void ParseOneStep()
    {
      ...
      if (_Stream.TryReadNextInGroup(out var value))
      {
        AppendEntry(value, suppressChildren: false, owner);
