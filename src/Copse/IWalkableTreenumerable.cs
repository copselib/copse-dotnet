...
namespace Copse
{
  ...
  /// source, the value itself for a source whose value is its own handle (the same split, and
  ...
  /// <see cref="ParentResult{THandle}"/>. Roots are the children of the virtual forest-root position
  ...
  public interface IWalkableTreenumerable<TValue, THandle> : ITreenumerable<TValue>
  {
    ...
    /// handle-to-value map; the identity when THandle is its own value).</summary>
