...
{
  /// <summary>
  /// The result of a parent pull: whether the node has a parent and, if so, which one. The upward
  ...
  ///
  /// <para>Unlike <see cref="ChildResult{TNode}"/> this carries a bare node, not a
  /// <see cref="NodeAndSiblingIndex{TNode}"/>: a child pull knows the sibling index it is handing
  ...
  /// </summary>
  public readonly struct ParentResult<TNode>
  {
    public ParentResult(TNode parent)
    {
      HasParent = true;
      Parent = parent;
    }
