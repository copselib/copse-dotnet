...
namespace Copse.Async
{
  ...
  public readonly struct AsyncTreeWalker<TValue, THandle>
  {
    ...
    public async ValueTask<Option<AsyncTreeWalker<TValue, THandle>>> MoveToParentAsync()
      => (await Topology.TryGetParentAsync(Focus).ConfigureAwait(false))
        .Map(Topology, (topology, parent) => new AsyncTreeWalker<TValue, THandle>(topology, parent));
    ...
    public async ValueTask<Option<AsyncTreeWalker<TValue, THandle>>> MoveToChildAsync(int childIndex)
      => (await Topology.TryGetChildAtAsync(Focus, childIndex).ConfigureAwait(false))
        .Map(Topology, (topology, child) => new AsyncTreeWalker<TValue, THandle>(topology, child.Node));
    ...
    public async ValueTask<Option<AsyncTreeWalker<TValue, THandle>>> MoveToRootAsync(int rootIndex)
      => (await Topology.TryGetRootAtAsync(rootIndex).ConfigureAwait(false))
        .Map(Topology, (topology, root) => new AsyncTreeWalker<TValue, THandle>(topology, root.Node));
