...
namespace Copse.Dags
{
  ...
  /// to it. Downward (<see cref="Dagnumerable.RootfixDispatch"/>): one per live in-edge, in
  /// discovery order, sources carrying the single seeded inflow on a default edge. Upward
  /// (<see cref="Dagnumerable.LeaffixDispatch"/>): one per live out-edge, in the pass's arrival
