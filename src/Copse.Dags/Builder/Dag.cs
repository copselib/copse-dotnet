...
namespace Copse.Dags
{
  ...
  /// the graph-theoretic vocabulary: sources and sinks) -- and the
  /// family's concrete <see cref="IDagnumerable{TNode, TEdge}"/>. Acquisition is LAZY: Kahn on demand over the live node graph, no snapshot,
  ...
  public sealed partial class Dag<TValue, TEdge>
  {
    ...
    public void SortChildrenBy<TKey>(Func<DagNode<TValue, TEdge>, TKey> keySelector)
    {
      ...
      foreach (var node in this.GetHandles())
