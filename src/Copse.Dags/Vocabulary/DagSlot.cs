...
namespace Copse.Dags
{
  ...
  /// the bind's law. More than one attachment is sharing, not duplication: on a dag the slot
  ...
  public readonly struct DagSlot<TEdge>
  {
    ...
    /// <summary>The general slot: any attachments, inside or outside, with or without payloads.</summary>
  ...
  /// <summary>One slot attachment: where the out-edges hang from (a fragment node, or the outside -- the original's in-edge parents) and the optional payload they compose through.</summary>
  public readonly struct DagSlotAttachment<TEdge>
  {
    private DagSlotAttachment(bool fromOutside, int fragmentNode, bool hasPayload, TEdge payload)
    {
    ...
    public static DagSlotAttachment<TEdge> Under(int fragmentNode) => new DagSlotAttachment<TEdge>(false, fragmentNode, false, default);
    public static DagSlotAttachment<TEdge> Under(int fragmentNode, TEdge payload) => new DagSlotAttachment<TEdge>(false, fragmentNode, true, payload);
    public static DagSlotAttachment<TEdge> FromOutside() => new DagSlotAttachment<TEdge>(true, -1, false, default);
    public static DagSlotAttachment<TEdge> FromOutside(TEdge payload) => new DagSlotAttachment<TEdge>(true, -1, true, payload);

    public override string ToString()
    {
      var origin = IsFromOutside ? "outside" : $"under {FragmentNode}";
      return HasPayload ? $"{origin} via {Payload}" : origin;
    }
  }
}
