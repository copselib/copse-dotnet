namespace Copse.Dags
{
  ...
  /// correlation key to per-edge pass results: a downward dispatch result's
  /// <see cref="DagDispatchResult{TNode, TDispatch}.Arrivals"/> sit in the same order, so
  /// <c>edge.Child.Arrivals[edge.InEdgeIndex]</c> is THIS edge's delivery -- no payload
