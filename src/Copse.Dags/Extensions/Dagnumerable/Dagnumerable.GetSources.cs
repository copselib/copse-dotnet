...
namespace Copse.Dags
{
  public static partial class Dagnumerable
  {
    ...
    /// last path, so no operator ever creates a mid-stream source (the same fact that keeps
    /// TakeDownstreamWhere a capture rather than a wrapper). Deferred: the walk
    /// is acquired on first enumeration -- and the early exit means a CYCLIC graph's sources
    /// still stream fine (cycles surface as starvation at exhaustion,
