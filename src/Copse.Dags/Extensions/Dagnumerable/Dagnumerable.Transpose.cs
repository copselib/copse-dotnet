namespace Copse.Dags
{
  ...
  {
    /// <summary>
    /// The orientation flip: the same nodes and edges with every arrow reversed, presented in
    /// the transpose's own topological order -- upward consumption is forward consumption of
    /// the transpose. <c>Materialize().Transpose()</c>,
    ...
    /// dispatch (<c>Transpose().Sourcefix().Dispatch(seed, survey)</c> is the seeded upward
    /// pass).
    /// </summary>
