...
| TakeSubtreesWhere | F, D, B (ALL stream) | same-dim | streams | **THE SCAN SPELLING (2026-08-17, the layering north star's first landing — SELECT_INTO_CAPTURES_DESIGN.md §5)**: the operator IS `RootfixScan(false, (kept, n) => kept \|\| predicate(n)).Where(pair => pair.Accumulate).Select(pair => pair.Node)` — "keep this node" is a rootfix fold fact, the outermost rule falls out of the disjunction's short-circuit, and the chain joins into ONE SelectWhere driver over the scan engine, so the result composes (a following Select lands in the driver's mapping). The former buffer arms are RETIRED — their "result's BFT cannot stream" rationale was DISPROVEN: general Where's breadth-first wrapper produces the re-rooted forest's true level order by pulling its inner ahead through its queue (verified on the reorder-wall shape: deep match preceding a shallow match in preorder — pinned in TakeSubtreesWhereTests). BREAKING (pre-beta): composite + B-narrow returned buffers through 2026-08-17; streaming semantics now (predicate per drain, Materialize = the consumer's pin). Cost trade measured at the swap: alloc collapses where the result store dominated (Triangle ~41MB→~0.2-0.5MB), composite time ~1.5x up (per-pull scan+driver vs buffered decode), DFT chain trades result store for the scan's O(depth) stack. D-narrow arm KEEPS its bespoke O(1)-state pass-through wrapper (leaner than the chain's O(depth); narrow results are outside the citizenship anyway). Semantics unchanged: <b>(2026-08-06 ruling)</b> matched subtrees re-rooted, match depth compresses to 0, descendants keep sibling indices, roots take source preorder match order, OUTERMOST MATCH WINS; predicate arity-split value \| positional (source labels; the positional flavor speaks the scan ENGINE's context accumulator directly — the public accumulator has no positional seat); the dag analog `TakeSubgraphsWhere` on experimental/dag gets outermost EMERGENTLY from induced in-degree, and is the general form |
