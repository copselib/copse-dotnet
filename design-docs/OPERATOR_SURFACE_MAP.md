...
| WithPosition | F, D, B | same-dim | streams | **STAGE A 2026-08-18 (feature/with-position, WITHPOSITION_DESIGN.md)**: the ONE positional operator — pairs each node with its position in ITS input tree (`ITreenumerable<NodeContext<TNode>>`), positionality as a SPELLING the value algebra composes over. Sugar over the positional Select pair, so it rides the light machinery from birth: in-tier merge over label-preserving chains, stacks over relabeling ones (the join rule), and the full positional spelling `WithPosition().PruneAfter(nc => …).Select(nc => nc.Node)` collapses to ONE middle-tier wrapper with the pair minted transiently inside the composed selector (the emission mint — pinned in WithPositionTests). Capture scope is explicit: pairs are DATA, not coordinates — a relabeling operator downstream does not update them (re-capture by calling again); this makes pre-relabel readings expressible, deliberately. Stages B-D (positional overloads become sugar; the arity axis machinery — join-rule guards, Relabels-from-the-interface, positional selector structs, context doors — deletes) are witness-gated, see the design doc |
