...
| LeaffixScan (seed \| leafNodeSelector) | D; **B**; F(→D) | ITreenumerableBuffer\<ScanResult\> | capture(deferred-once) | **THE DUAL RESHAPE 2026-08-05** (design-docs/SCANRESULT_DESIGN.md THE LEAFFIX DUAL): `(seed \| leafSelector \| positional, Func<TAcc,TAcc,TAcc> edgeAccumulator, Func<TAcc,TSource,TAcc> nodeAccumulator)` — value(n) = nodeAcc(edgeReduce(children), n); the nodeAccumulator IS RootfixScan's fold shape; the seed = the VIRTUAL FRINGE's arrival, participating (value(leaf) = nodeAcc(seed, leaf)); selector = bypass; seed ≠ constant selector, pinned; the old map-then-combine (map fused boundary+contribution) is DELETED, ternary context flavor awaits an edgeAccumulator workload; sugar over LeaffixDispatch's internal pass (CrossTierCoherenceTests pins both flavor pairs). **RECEIVER-SMART 2026-08-14, seed flavor only** (the experiment collapse): a preorder-affording capture folds IN PLACE — concrete buffer → the span fold over its raw store (reverse-ordinal, span hops, no child-index/positions builds; measured 3.3–4.1x on buffer receivers), memo → the walker-probe fold (completes it exactly once, no second skeleton); level-order captures and streams take the engine; the in-place result buffer carries probes at birth (ReceiverSmartOperatorTests pins every receiver shape against the engine oracle). Bypass/positional flavors stay engine-only (they need positions) |
