...
- **Hand-written sync that remains:** `TreenumeratorBase`/`Wrapper`, `EmptyTreenumerator`,
  ...
  (Since the de-share, the store SPIs, read structs, and completed array stores are all
  generated — their async sources are the single source of truth.)
...
- **Sync-only surface (no async twin today):** `EnumerableExtensions.ToDegenerateTree` and
  `.ToTrivialForest` — the flat-sequence doors; an `IAsyncEnumerable` lift is future surface,
  not generated.
